...
class PlayerI(drobots.Player):
	...
	def makeController(self, bot, current):
		...
		print("JUGADOR 2: "+str(self.name)+"--> creando controlador: "+str(self.cont))
		proxisFactoria=list(self.containerFactorias.list().values())
		if(bot.ice_ids() == ['::Ice::Object', '::drobots::Attacker', '::drobots::RobotBase']): 
			print("JUGADOR 2-->Creando controlador del atacante usando factoría: "+str(proxisFactoria[self.cont]))
			...
			#print("JUGADOR 2-->Creado robot atacante: "+str(controllerAtacante))    
			...
			print("JUGADOR 2-->Creando controlador del defensor usando factoría: "+str(proxisFactoria[self.cont]))
			factoria=drobotsAux.controllerFactoryPrx.uncheckedCast(proxisFactoria[self.cont]) 
			controllerDefensor=factoria.make(bot,"defensor",self.containerMinas,self.containerController,self.name) 
			#print("JUGADOR 2-->Creado robot defensor: "+str(controllerDefensor))
			...
			print("JUGADOR 2-->Creando controlador del completo usando factoría: "+str(proxisFactoria[self.cont]))
			factoria=drobotsAux.controllerFactoryPrx.uncheckedCast(proxisFactoria[self.cont]) 
			controllerCompleto=factoria.make(bot,"completo",self.containerMinas,self.containerController,self.name)
			#print("JUGADOR 2-->Creado robot completo: "+str(controllerCompleto)) 
	...
	def makeDetectorController(self, current):
		print("JUGADOR 2: "+str(self.name)+"--> creando controlador detector: "+str(self.contDetec))
		proxisFactoria=list(self.containerFactorias.list().values())
		print("JUGADOR 2-->Creando controlador del detector usando factoría: "+str(proxisFactoria[self.contDetec]))
		...
		#print("JUGADOR 2-->Creado robot detector: "+str(controllerDetector)) 
	...
	def getMinePosition(self, current):
		print("JUGADOR 2: "+str(self.name)+"--> creando mina: "+str(self.contMin))
